...
def parse_arguments(args=None):
    ...
    parser.add_argument("--auto", "--all", "-a", dest="auto", action="store_true",
                        help="Automatically find all source files in data/ and write C header files to src/data/")
...
def get_context(infile, outfile):
    ...
    dir, name, type = (os.path.basename(os.path.dirname(infile)), os.path.basename(infile).split(os.path.extsep)[0],
                       os.path.basename(infile).split(os.path.extsep)[-1])
    ...
    if os.path.isdir(outfile):
        ...
        outfilename = "%s%s.h" % (name.capitalize(), type.upper())
...
def main(args):
    args.sources = os.path.realpath(
        args.sources or os.sep.join((os.path.dirname(os.path.realpath(__file__)), "..", "data")))
    args.target = os.path.realpath(
        args.target or os.sep.join((os.path.dirname(os.path.realpath(__file__)), "..", "src/data")))
    check_args(args)
